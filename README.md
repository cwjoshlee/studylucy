...
3. 로컬 비교 채점: 한글 읽기는 다시 읽을 표현을 붉은 굵은 글씨로 표시.
4. 수학 채점: 이야기 속 1~20 덧셈 문제를 읽은 뒤 답 입력, `PASS` 또는 `FAIL`로 표시.
5. Synology NAS: 집 안에서 직접 호스팅하고 싶을 때의 대안.
...
- 수학 문제는 지문과 문제를 먼저 읽어야 답 입력칸이 열리고, 답 입력 후 `PASS` 또는 `FAIL`로 채점 내역을 표시합니다.

...
6. 오늘의 문제 세트: 날짜가 바뀌면 문제 순서와 당일 진도를 새로 시작.
7. Synology NAS: 집 안에서 직접 호스팅하고 싶을 때의 대안.
...
- 읽기 시간은 최대 1분이며, 아이가 다 읽고 `읽기 완료`를 누르면 채점합니다.
...
- 문제는 날짜를 기준으로 매일 다른 순서로 제시하며, 같은 날 새로고침해도 같은 순서를 유지합니다.
- 학습 진도와 다시 볼 표현은 오늘 날짜 기준으로 저장되어 다음 날 새 문제 세트를 시작합니다.

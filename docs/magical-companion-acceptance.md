...
| localhost 브라우저 | 아래 Task 8 headed Chromium fresh rerun 기록 | PASS |
...
- 검증한 구현 HEAD: `bc426abea95f7f2689b1463079b4401775c91953`
- 최종 QA 실행: `20260717T021042Z-22203`, fresh DB를 사용한 새 headed Chromium 세션, localhost Vite `5173` + Fastify `8787`
- authority 발급 경계 확인: `POST /api/student/learning-sessions`를 Playwright route로 지연한 동안 role=status `학습 준비 상태`와 `학습을 준비하고 있어요. 잠깐 기다려 주세요.`만 표시되고, 친구 말풍선·이야기·힌트·수동 transcript·수학 답·제출·다음 조작은 없거나 사용할 수 없었음. 지연 해제 뒤 실제 201을 받고 정상 lesson이 활성화됐으며 route를 제거함
...
| 1368×912 reduced motion 정답 | `output/playwright/correct-reduced-motion.png` | 새 별 적립 가능 `거꾸로 붙은 이름표` 정답에서 서버 receipt에 따라 별 보상은 표시되지만 star particle은 0개이고 보상 요소의 계산된 transform은 `none`이며 `data-reduced-motion=true`임 |

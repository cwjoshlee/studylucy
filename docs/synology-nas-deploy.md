...
2026-07-19 현재 Node 22/npm 11.11.0 전체 검사에서 42개 테스트 파일·721개 테스트와 production 빌드가 통과했다. `sua-learning:final-compatibility-smoke` (`sha256:62fa3aa9281ba6fc90fa11ffd5b275df56f4361d1bd21ffc96a32607a3cccfd4`)는 `linux/amd64`로 빌드했고, loopback 임시 컨테이너는 정확히 `{"status":"ok"}`를 반환한 뒤 제거됐다. 다음 경계는 이 로컬 회귀검사와 컨테이너 smoke에서 확인했으며, NAS 운영 증거나 실제 배포 승인은 아니다.
...
- [x] AI 학습실의 첫 설정 읽기가 일시 실패하면 설정·문제 생성 화면에서 접근 가능한 재시도가 남는다. 진행 중인 수동 재시도와 늦은·언마운트된 응답은 적용하지 않으며, 성공한 설정만 문제 생성 준비 상태를 연다.
- [x] `/settings/view` 404 뒤 더 오래된 제공자 배열에 토큰 요금이 없으면 실제 `ai-coach-settings`의 월 예산·사용액만 사용한다. 이전 서버에는 예산·요금 편집을 렌더하지 않아 가짜 요금이나 자동 쓰기 요청이 생기지 않는다.

이 여섯 항목을 포함한 새 전체 브랜치 검토와 그 결과를 확인한 뒤 받는 새 명시적 릴리스 승인은 여전히 필수다. 그 승인 전에는 push, merge, image publish 또는 NAS 자동 배포를 시작하지 않는다.
